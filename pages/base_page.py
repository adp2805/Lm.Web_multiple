...
class Base_page(Browser):
    ...
    def verify_err_msg_outline(self,err,element):
        actual_error_message = self.driverObject.find_element(*element).text.strip()
        assert err.strip() == actual_error_message, f"Expected error message:{err}\
        is different from {actual_error_message}"
